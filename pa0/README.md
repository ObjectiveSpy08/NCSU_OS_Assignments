...
Grade: 90/100
Comments: 
Task2: -4 (Atleast for preceding and after values, you have to dereference using *(&etext - 1) and so on). Task3: -4 (It is ebp + 2 for before. Also while printing upto 4 stack locations, you need to check that it is "<" ebp and not let it cross it that too starting from esp). Task4: -2 (You need to even check that pstate is not PRFREE. Your printing is even happening for killed processes). 
